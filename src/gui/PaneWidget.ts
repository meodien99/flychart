...
import { TouchMouseEvent, MouseEventHandler, Position } from "./MouseEventHandler";
...
export class PaneWidget implements IDestroyable {
    ...
    public setPriceAxisSize(width: number): void {
        ensureNotNull<PriceAxisWidget>(this._priceAxisWidget).setSize(new Size(width, this._size.h));
    }
    ...
    public recalculatePriceScale(): void {
        const pane = ensureNotNull<Pane>(this._state);
    ...
    private _drawGrid(ctx: CanvasRenderingContext2D): void {
        const state = ensureNotNull<Pane>(this._state);
