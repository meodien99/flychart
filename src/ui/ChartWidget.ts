...
import { InvalidateMask, InvalidateLevel } from "../models/InvalidateMask";
...
import { SEPARATOR_HEIGHT, PaneSeparator } from "./PaneSeparator";
...
export class ChartWidget implements IDestroyable {
    ...
    private _drawRafId: number = 0;
 	...
 	public destroy(): void {
 		...
 		ensureNotNull<TimeAxisWidget>(this._timeAxisWidget).destroy();
 	...
 	public resize(height: number, width: number, forceRepaint: boolean = false): void {
 		...
 		ensureNotNull<HTMLElement>(this._element).style.height = heightStr;
 		ensureNotNull<HTMLElement>(this._element).style.width = widthStr;

 		this._tableElement.style.height = heightStr;
 		this._tableElement.style.width = widthStr;

 		if (forceRepaint) {
 			this._drawImpl(new InvalidateMask(InvalidateLevel.Full));
 		} else {
 	...
 	public paint(invalidateMask?: InvalidateMask): void {
 		if (invalidateMask === undefined) {
 			invalidateMask = new InvalidateMask(InvalidateLevel.Full);
 		}
 	...
 	private _adjustSizeImpl(): void {
 		...
 		for (const paneWidget of this._paneWidgets) {
 			if (this._options.priceScale.position !== 'none') {
 				priceAxisWidth = Math.max(priceAxisWidth, ensureNotNull<PriceAxisWidget>(paneWidget.priceAxisWidget()).optimalWidth());
 			}
 	...
 	private _drawImpl(invalidateMask: InvalidateMask): void {
 		...
 		if (invalidationType === InvalidateLevel.Full) {
 			this._updateGui();
 		...
 		} else if (invalidationType === InvalidateLevel.Light) {
 			if (invalidateMask.getFitContent()) {
 				this._model.timeScale().fitContent();
 			}
 			this._timeAxisWidget.update();
