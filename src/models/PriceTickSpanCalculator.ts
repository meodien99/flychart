...
export class PriceTickSpanCalculator {
    ...
    public tickSpan(high: number, low: number, maxTickSpan: number): number {
        ...
        while (true) {
            ...
            const resultTickSpanLargerMinMovement = greaterOrEquals(resultTickSpan, minMovement, tickSpanEpsilon) && resultTickSpan > (minMovement + tickSpanEpsilon);
        ...
        if ((this._fractionalDividers.length > 0) && equal(resultTickSpan, 1, tickSpanEpsilon)) {
            ...
            while (greaterOrEquals(resultTickSpan, maxTickSpan * c, tickSpanEpsilon) && resultTickSpan > (minMovement + tickSpanEpsilon)) {
                resultTickSpan /= c;
